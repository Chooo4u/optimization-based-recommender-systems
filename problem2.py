...
def cosine_similarity(RA, RB):
    ...
    # cosRA_RB = np.dot(RA, RB) / math.sqrt(RA ** 2 * RB ** 2)
    ...
    for i in range(len(RA)):
        ...
        if A != 0 and B != 0:
            ...
            # sumBB += B*B
            # sumAB += A*B
            # S = sumAB / math.sqrt(sumAA * sumBB)
            ra.append(RA[i])
            rb.append(RB[i])
        else:# if the two user share no item in their ratings
            ra.append(0)
            rb.append(0)
        # compute cosine similarity on the shared items
    P = sum(np.multiply(np.array(ra),np.array(rb)))
    sumAA = sum(A * A for A in ra)
    sumBB = sum(B * B for B in rb)
    if sumAA == 0 or sumBB == 0:
        S = 0
    else:
        S = P / (np.sqrt(sumAA) * np.sqrt(sumBB))
    S = float(S)
    #########################################
    return S
...
def user_similarity(R, j, idx):
    ...
    for x in idx:
        Ridx = R[:,x]
        us = float(cosine_similarity(Rj, Ridx))
        sim.append(us)
    #########################################
    return sim 
...
def user_based_prediction(R, i_movie, j_user, K=5):
    ...
    simx = zip(sim, x_movie)
    sim_new = sorted(simx, key=lambda v:v[0],reverse=True)

    sum_nup = 0
    sum_ndown = 0
    for n in range(0, K):
        sum_nup += sim_new[n][0] * sim_new[n][1]
        sum_ndown += sim_new[n][0]
    if len(idx) == 0:
        p = 3
    else:
        p = float(sum_nup / sum_ndown)
    #########################################
    return p 
...
def compute_RMSE(ratings_pred, ratings_real):
    ...
    for i in range(len(ratings_pred)):
        sum_x += (ratings_pred[i] - ratings_real[i]) ** 2
        n += 1
    RMSE= math.sqrt(sum_x / n)
...
def load_rating_matrix(filename = 'movielens_train.csv'):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    R = np.zeros(shape = (1682, 943))
    umr = np.loadtxt(filename, delimiter = ',')
    for u in range(0, 943):
        for i in range(len(umr)):
            if umr[i,0] == u + 1:
                m = int(umr[i,1]) - 1
                r = int(umr[i,2])
                R[m,u] = r
...
def load_test_data(filename = 'movielens_test.csv'):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    m_ids = list(np.genfromtxt(filename, dtype = int, delimiter=",", usecols=(1,),unpack = True))
    for i in range(len(m_ids)):
        m_ids[i] = int(m_ids[i] - 1)
    u_ids = list(np.loadtxt(filename, dtype = int, delimiter=",", usecols=(0,),unpack = True))
    for j in range(len(u_ids)):
        u_ids[j] = int(u_ids[j] - 1)
    ratings = list(np.loadtxt(filename, dtype = float, delimiter=',', usecols=(2,)))
    for k in range(len(ratings)):
        ratings[k] = float(ratings[k])
...
def movielens_user_based(train_file='movielens_train.csv', test_file ='movielens_test.csv', K = 5):
    ...
    # for u in u_ids:
    #     for m in m_ids:
    for iii in range(len(m_ids)):
        i_movie = int(m_ids[iii])
        j_user = int(u_ids[iii])
        r_pred = user_based_prediction(R, i_movie, j_user, K)
        ratings_pred.append(r_pred)
    #########################################
    # compute RMSE
