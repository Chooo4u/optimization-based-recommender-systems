...
def eval_player(ID='hattesc01'):
    ...
    grades = df.loc[df['playerID'].isin([ID])]
    BA = grades['H'] / grades['AB']
    OBP = (grades['H'] + grades['BB'] + grades['HBP']) / (grades['AB'] + grades['BB'] + grades['HBP'] + grades['SF'])
    TB = grades['H'] + grades['2B'] + grades['3B'] *2 + grades['HR'] * 3
    SLG = TB / grades['AB']
...
def salary(ID='hattesc01'):
    ...
    Slist = df.loc[df['playerID'].isin([ID])]
    S = Slist['salary']
